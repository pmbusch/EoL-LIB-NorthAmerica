...
ev <- ev %>%
  ...
  rowwise() %>%
  mutate(
    kWh_addLIB = sum(as.numeric(add_LIB_vector) * as.numeric(kwh_veh_vector)),
...
ss_vector <- ev %>%
  ...
  group_by(Country, Year, Vehicle, age) %>%
  reframe(kwh = sum(kWh_available)) %>%
  ungroup() %>%
  filter(kwh > 0)
...
lib_recycling <- ev %>%
  ...
  mutate(kwh = if_else(str_detect(Flow, "available"), kwh * (1 - p.share_repurpose), kwh)) %>%
...
if (considerTrade) {
  ...
  lib_recycling_trade <- lib_recycling %>%
    ...
    mutate(kwh = kwh * Share, Country = if_else(To == "RoW", "Exports", To), Share = NULL, To = NULL) %>%
    group_by(Vehicle, Country, Year, Flow) %>%
    reframe(kwh = sum(kwh)) %>%
    ungroup()

  lib_recycling <- rbind(filter(lib_recycling, Vehicle %in% large_veh), lib_recycling_trade)
}


# add to results
ev <- ev %>% filter(!str_detect(Flow, "available|recycling")) %>% rbind(lib_recycling)
