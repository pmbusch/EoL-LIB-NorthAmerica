  ...
  for (b in lib_scens) {
    ...
    for (e in eol_scens) {
      ...
      for (tra in trade_scens) {
        ...
        # Join
        df <- rbind(ev, ss, ce) %>% group_by(Vehicle, Country, Year, Flow) %>% reframe(kwh = sum(kwh)) %>% ungroup()

        # Production Scrap
        unique(df$Flow)
        df <- df %>% mutate(FlowType = if_else(Flow == "LIB_recycling", "LIB Recycling", "LIB Production"))
        ...
        df <- df %>%
          left_join(params) %>%
          mutate(
            blackMass_kg = kwh / energy_dens * cathode_we_frac,
            battery_kg = kwh / energy_dens,
            energy_dens = NULL,
            cathode_we_frac = NULL
          )
