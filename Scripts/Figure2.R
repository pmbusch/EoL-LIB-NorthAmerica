...
ggsave("Figures/flows_EV.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7 * 1.5)
...
p1 <- ggplot(data_fig, aes(Year, gwh, fill = Vehicle)) +
  ...
  theme(panel.spacing = unit(0.5, "cm"), plot.tag = element_text(face = "bold"))
...
data_fig2 <- data_fig0 %>%
  ...
  mutate(share = gwh / sum(gwh)) %>%
  mutate(Country = factor(Country, levels = rev(c("United States", "Canada", "Mexico", "Exports"))))
...
p2 <- ggplot(data_fig2, aes(Year, share, fill = Country)) +
  ...
  scale_fill_manual(
    values = c("United States" = "#3C3B6EFF", "Mexico" = "#006847FF", "Canada" = "#B22222FF", "Exports" = "#999999FF")
  ) +
