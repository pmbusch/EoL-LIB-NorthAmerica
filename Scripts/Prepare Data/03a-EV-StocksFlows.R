...
for (s in unique(sales$Scenario)) {
  ...
  for (lif in lifetime_scen) {
    ...
    for (re in reuse_scens) {
      ...
      stock <- stock %>% rowwise() %>% mutate_if(is.list, ~ paste(unlist(.), collapse = '|'))
      write.csv(stock, paste0("Inputs/Stocks/", s, "_", lif, "_reuse", round(re * 100, 0), ".csv"), row.names = F)
