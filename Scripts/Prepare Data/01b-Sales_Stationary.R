...
unique(df_ren$history)
unique(df_ren$tableName)
unique(df_ren$seriesId)
unique(df_ren$scenario)

## Filter data ---------
# filter by generation data
df_ren <- df_ren %>% filter(substr(seriesId, 0, 4) == "cap_") %>% filter(str_detect(seriesName, "Electric Power"))
unique(df_ren$seriesName)
unique(df_ren$unit) # GW

# solar and wind
df_ren <- df_ren %>% filter(str_detect(seriesName, "Wind|Solar"))
...
usa_hist <- read_excel(paste0(url_drive, "USA_existcapacity_annual.xlsx"), skip = 1)
head(usa_hist)
usa_hist <- usa_hist %>%
  filter(Year > 2014) %>%
...
usa_hist <- usa_hist %>%
  group_by(Year) %>%
  reframe(gw = sum(`Nameplate Capacity (Megawatts)`) / 1e3) %>%
  ungroup() %>%
...
mex <- read_excel(paste0(url_drive, "Mexico_ElectricityCap_PROSEDEN.xlsx"), sheet = "Forecast", range = "G5:AG7")
...
head(can)
unique(can$Variable)
can <- can %>% filter(Scenario == "Current Measures", Region == "Canada") %>% filter(str_detect(Variable, "Solar|Wind"))

can <- can %>% group_by(Year) %>% reframe(gw = sum(Value) / 1e3) %>% ungroup() # MW to GW
...
df %>%
  ...
  mutate(key = if_else(key == "gw", "Added year-to-year capacity (GW)", "Total installed capacity (GW)")) %>%
  ggplot(aes(Year, value, fill = Country)) +
  geom_col(position = "stack") +
  facet_wrap(~key, ncol = 1, scales = "free_y") +
  labs(x = "", y = "", title = "Solar and Wind Electricity Capacity (GW)")

ggsave("Figures/Inputs/gw_capacity.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
...
df <- df %>%
  ...
  reframe(gw = sum(gw), gw_stock = sum(gw_stock)) %>%
  ungroup() %>%
  mutate(gwh = gw * sg * sd, gwh_stock = gw_stock * sg * sd, gw_stock = NULL, gw = NULL)

df %>% filter(Year %% 5 == 0) %>% pivot_wider(names_from = Country, values_from = c(gwh, gwh_stock))
