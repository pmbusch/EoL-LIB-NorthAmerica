...
bat <- bat %>% filter(Sales_Country %in% c("Mexico", "Canada", "USA")) %>% filter(Propulsion == "BEV")
...
bat <- bat %>%
  dplyr::select(
    ...
    Brand,
    Global_Segment,
    `Estd_Average_Battery_Capacity_[kWh]`,
    Primary_Application,
    Cathode_Chemistry,
    `2010`,
    `2011`,
...
bat <- bat %>% mutate(year = as.numeric(year)) %>% filter(year < 2025, year > 2014, sales > 0)
unique(bat$year)
range(bat$sales)
...
ggsave(
  "Figures/Inputs/EVVol_dist_MDV.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 2,
...
bat_scen <- dist %>%
  ...
  reframe(
    mean_kwh_veh = weighted.mean(kwh_veh, sales),
...
bat <- bat %>%
  ...
  reframe(kwh_veh = weighted.mean(`Estd_Average_Battery_Capacity_[kWh]`, sales), sales = sum(sales)) %>%
  ungroup()
...
bat <- bat %>%
  ...
  mutate(kwh_avg = weighted.mean(kwh_veh, sales), kwh_share = kwh_total / sum(kwh_total)) %>%
  ungroup() %>%
  mutate(kwh_veh_chem = kwh_share * kwh_avg, kwh_total = NULL, kwh_share = NULL)
...
# historical sales
sales <- bat %>% group_by(Vehicle, Year, Sales_Country) %>% reframe(sales = sum(sales)) %>% ungroup()
...
bat_country_size <- bat %>%
  ...
  reframe(kwh_veh_total = mean(kwh_veh_total)) %>%
  ungroup()
...
bat_country_chem <- bat %>%
  group_by(Vehicle, Year, Sales_Country) %>%
  mutate(chem_share_mwh = kwh_veh / sum(kwh_veh)) %>%
  ungroup()
...
sales %>%
  ...
  ggplot(aes(Year, sales, col = Sales_Country, group = key)) +
  geom_line(aes(linetype = Vehicle)) +
  geom_point() +
  coord_cartesian(expand = F) +
  labs(x = "", y = "", title = "Vehicle Sales [thousands]", col = "")
ggsave(
  "Figures/Inputs/Historical_HDV_EVSales.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 2,
...
ggsave("Figures/Inputs/HDV_LibSize.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
...
ggsave("Figures/Inputs/HDV_LibChem.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
