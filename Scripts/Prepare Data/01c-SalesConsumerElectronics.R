...
gdp <- read_excel("Inputs/Original/P_Data_Extract_From_World_Development_Indicators.xlsx",
                  sheet = "Data",range="A1:P218")
names(gdp)
gdp <- gdp %>% 
...
recyc <- ce %>%
  ...
  reframe(ce_gwh_recyc=sum(ce_gwh_recyc)) %>% ungroup() %>% 
