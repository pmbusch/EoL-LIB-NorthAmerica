...
head(bat)
bat <- bat %>% rename(Country = Sales_Country) %>% dplyr::select(Vehicle, Year, Country, chemistry, kwh_veh)
...
bat <- bat %>% group_by(Vehicle, Year, Country) %>% reframe(kwh_veh = sum(kwh_veh)) %>% ungroup()
...
# no chemistry for modeling
bat_HDV <- bat_HDV %>% group_by(Vehicle, Year, Country) %>% reframe(kwh_veh = sum(kwh_veh)) %>% ungroup()
...
# load data
bat_scen <- rbind(read.csv("Inputs/Battery/bat_quantiles.csv"), read.csv("Inputs/Battery/bat_quantiles_MDV.csv"))
...
slope_bat <- bat_scen %>%
  ...
  mutate(
    slope_low = (low_kwh_veh - mean_kwh_veh) / (goal_year - 2024),
...
ggsave("Figures/Inputs/BatterySize.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
...
x %>%
  ...
  group_by(Vehicle, Year, Country, chemistry) %>%
...
ggsave(
  "Figures/Inputs/BatterySizeChem.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 2,
