...
head(bat)
# sum over years
dist <- bat %>% group_by(Sales_Country, Vehicle, type, year, rowid) %>% reframe(value = sum(value)) %>% ungroup()
dist <- dist %>% pivot_wider(names_from = type, values_from = value) %>% filter(!is.na(Mwh))
...
bat_scen <- dist %>%
  ...
  reframe(
    mean_kwh_veh = weighted.mean(kwh_veh, Reg),
...
bat <- bat %>%
  group_by(Sales_Country, Vehicle, Cathode_Chemistry, Cathode_Mix, type, year) %>%
  reframe(value = sum(value))
bat <- bat %>% pivot_wider(names_from = type, values_from = value) %>% rename(MWh = Mwh, unit = Reg)
...
bat %>%
  filter(year == 2024) %>%
  group_by(Vehicle, Cathode_Chemistry) %>%
  summarise(x = sum(MWh, na.rm = T)) %>%
  arrange(desc(x)) %>%
  ungroup() %>%
  mutate(perc = x / sum(x) * 100)
bat %>%
  filter(year == 2024) %>%
  group_by(Vehicle, Cathode_Mix) %>%
  summarise(x = sum(MWh, na.rm = T)) %>%
  arrange(desc(x)) %>%
  ungroup() %>%
  mutate(perc = x / sum(x) * 100)
...
# Checks
bat %>% group_by(year) %>% summarise(MWh = sum(MWh), unit = sum(unit, na.rm = T)) %>% mutate(kWh_veh = MWh * 1e3 / unit)
...
bat_nmc <- bat_nmc %>%
  ...
  mutate(MWh = MWh * perc, unit = unit * perc, perc = NULL, x = NULL)
...
bat_country <- bat %>%
  ...
  summarise(MWh = sum(MWh, na.rm = T), unit = sum(unit, na.rm = T)) %>%
  ungroup()
...
bat_country <- bat_country %>%
  ...
  mutate(kwh_veh = sum(MWh) * 1e3 / sum(unit)) %>%
  ungroup() %>%
  mutate(kwh_veh = kwh_veh * share_units) %>%
  group_by(Vehicle, year, Sales_Country) %>%
  mutate(kwh_veh_total = sum(kwh_veh)) %>%
  ungroup()

bat_country <- bat_country %>% rename(Year = year)

bat_country %>% filter(Year == 2024) %>% group_by(Sales_Country, Vehicle) %>% reframe(x = mean(kwh_veh_total))
...
bat_country_size <- bat_country %>%
  group_by(Vehicle, Year, Sales_Country) %>%
  reframe(MWh = sum(MWh), unit = sum(unit)) %>%
  ungroup() %>%
...
sales %>%
  ...
  ggplot(aes(Year, unit, col = Sales_Country, group = key)) +
  geom_line(aes(linetype = Vehicle)) +
  geom_point() +
  coord_cartesian(expand = F) +
  labs(x = "", y = "", title = "Vehicle Sales [thousands]", col = "")
ggsave(
  "Figures/Inputs/Historical_EVSales.png",
  ggplot2::last_plot(),
  units = "cm",
  dpi = 600,
  width = 8.7 * 2,
...
bat_country_size %>%
  mutate(key = paste0(Sales_Country, Vehicle)) %>%
  ggplot(aes(Year, kwh_veh_total, col = Sales_Country, group = key)) +
  geom_line(aes(linetype = Vehicle)) +
  geom_point() +
  coord_cartesian(expand = F) +
  labs(x = "", y = "", title = "Avg. Battery Size [kWh/veh]", col = "")

ggsave("Figures/Inputs/LibSize.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
...
ggsave("Figures/Inputs/LibChem.png", ggplot2::last_plot(), units = "cm", dpi = 600, width = 8.7 * 2, height = 8.7)
